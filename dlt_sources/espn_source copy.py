...
def espn_mens_college_basketball_source(
        ...
        try:
            ...
            for page_of_refs in list_client.paginate(
                ...
                if fetched_event_details_for_page:
                    for event_detail_item in fetched_event_details_for_page:
                        yield event_detail_item  # Yield individual event details
    ...
    @dlt.transformer(
        ...
        data_from=events_transformer,
        write_disposition="merge",
        primary_key=["event_id_fk", "team_id"],
    )
    def event_competitors_transformer(event_detail: dict[str, Any]) -> Iterable[dict[str, Any]]:
        """
        Extracts competitor data from an event_detail object.
        Each event typically has one competition, and that competition has two competitors.
        Yields one dictionary per competitor.
        """
        event_id = event_detail.get("id")
        if not event_id:
            logger.warning(
                f"Event detail missing 'id'. Skipping competitors. Detail: {event_detail}"
            )
            return
        ...
        if not competitions or not isinstance(competitions, list) or len(competitions) == 0:
            logger.warning(
                f"Event detail for event_id '{event_id}' missing 'competitions' array or it's empty. "
                f"Skipping competitors. Detail: {event_detail}"
            )
            return

        # Assuming the first competition is the relevant one for NCAA basketball games
        competition = competitions[0]
        competitors_data = competition.get("competitors")

        if not competitors_data or not isinstance(competitors_data, list):
            logger.warning(
                f"Competition data for event_id '{event_id}' missing 'competitors' array. "
                f"Skipping competitors. Competition: {competition}"
            )
            return

        competitors_yielded_count = 0
        for competitor_entry in competitors_data:
            team_id_from_competitor = competitor_entry.get("id")
            if not team_id_from_competitor:
                logger.warning(
                    f"Competitor entry for event_id '{event_id}' missing 'id' (team_id). "
                    f"Skipping competitor. Entry: {competitor_entry}"
                )
                continue

            # Prepare the record to yield
            competitor_record = {
                "event_id_fk": str(event_id),
                "team_id": str(team_id_from_competitor),
                "uid": competitor_entry.get("uid"),
                "order": competitor_entry.get("order"),
                "homeAway": competitor_entry.get("homeAway"),
                "winner": competitor_entry.get("winner"),
                # Store $refs for subsequent transformers
                "team_ref": competitor_entry.get("team", {}).get("$ref"),
                "score_ref": competitor_entry.get("score", {}).get("$ref"),
                "linescores_ref": competitor_entry.get("linescores", {}).get("$ref"),
                "statistics_ref": competitor_entry.get("statistics", {}).get("$ref"),
                "leaders_ref": competitor_entry.get("leaders", {}).get("$ref"),
                "roster_ref": competitor_entry.get("roster", {}).get("$ref"),
                "records_ref": competitor_entry.get("records", {}).get("$ref"),
                # curatedRank might be nested, e.g., competitor_entry.get("curatedRank", {}).get("current")
                # For now, let's keep it simple or check sample data for exact structure
            }

            # Include curated rank if present (example, adjust based on actual data structure)
            curated_rank = competitor_entry.get("curatedRank", {})
            if isinstance(curated_rank, dict):
                competitor_record["curated_rank_current"] = curated_rank.get("current")

            yield competitor_record
            competitors_yielded_count += 1

        if competitors_yielded_count > 0:
            logger.info(
                f"Processed {competitors_yielded_count} competitors for event_id '{event_id}'."
            )
        elif not competitors_data:  # Only log if competitors_data was missing/empty initially
            pass  # Already logged above
        else:  # competitors_data was present but no valid competitors were found
            logger.info(f"No valid competitor entries found to process for event_id '{event_id}'.")
    ...
    def event_scores_transformer(competitor_detail: dict[str, Any]) -> Iterable[dict[str, Any]]:
        """
        Fetches the final score for a team in an event using the score_ref
        from the competitor_detail.
        Yields a dictionary with score information.
        """
        event_id_fk = competitor_detail.get("event_id_fk")
        team_id_fk = competitor_detail.get(
            "team_id"
        )  # from competitor_detail, this is the team's ID
        score_url = competitor_detail.get("score_ref")

        if not event_id_fk or not team_id_fk:
            logger.warning(
                "Competitor detail missing 'event_id_fk' or 'team_id'. "
                f"Skipping score fetching. Detail: {competitor_detail}"
            )
            return

        if not score_url:
            logger.info(  # Info, as it might be legitimately missing for some data
                f"No 'score_ref' found for event_id_fk '{event_id_fk}', team_id_fk '{team_id_fk}'. "
                "Skipping score fetching for this competitor."
            )
            return

        logger.info(
            f"Fetching score for event_id_fk '{event_id_fk}', team_id_fk '{team_id_fk}' from {score_url}"
        )
        try:
            response = detail_client.get(score_url)
            response.raise_for_status()
            score_data = response.json()

            # The API might return just the score value or a more complex object.
            # Based on sample: events_-event-id-_competitions_-competition-id-_competitors_-competitor-id-_score_example.json
            # it seems to be like: {"value": 70, "displayValue": "70", "winner": true/false}
            score_value = score_data.get("value")
            score_display_value = score_data.get("displayValue")
            score_winner_flag = score_data.get("winner")  # Optional

            if score_value is not None:  # score_value is the critical piece of information
                score_record = {
                    "event_id_fk": str(event_id_fk),
                    "team_id_fk": str(team_id_fk),
                    "score_value": score_value,
                    "score_display_value": score_display_value,
                }
                if score_winner_flag is not None:
                    score_record["score_winner_flag"] = score_winner_flag

                yield score_record
                logger.info(
                    f"Successfully fetched and yielded score for event_id_fk '{event_id_fk}', "
                    f"team_id_fk '{team_id_fk}'."
                )
            else:
                logger.warning(
                    f"Score data from {score_url} for event_id_fk '{event_id_fk}', "
                    f"team_id_fk '{team_id_fk}' did not contain a 'value'. Data: {score_data}"
                )

        except requests.exceptions.HTTPError as he:
            response_text = he.response.text if he.response else "No response body"
            logger.error(
                f"HTTPError fetching score from {score_url} "
                f"(event_id_fk '{event_id_fk}', team_id_fk '{team_id_fk}'): {he}. "
                f"Response: {response_text}"
            )
        except Exception as ex:
            logger.error(
                f"Unexpected error fetching score from {score_url} "
                f"(event_id_fk '{event_id_fk}', team_id_fk '{team_id_fk}'): {ex}"
            )
